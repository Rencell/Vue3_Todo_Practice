<template>
    ...
    <div  tabindex="-1" aria-modal="true" role="dialog" :class="myModal ? 'flex' : 'hidden'" class="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-0"></div>
...
const props = defineProps(['myModal', 'modalValue'])
const emit = defineEmits(['closeModal', 'send-my-list'])

function updateModal(id, updatedItem){
    emit('send-my-list', id, updatedItem)
    closeModal()
}
function closeModal(){
    emit('closeModal')
